...
export default function SuburiCounter() {
  ...
  function handleAngles(pose: PoseNetPose["pose"]) {
    ...
    elbow.right = calculateMidAngle(
      [rightShoulder.x, rightShoulder.y],
      [rightElbow.x, rightElbow.y],
      [rightWrist.x, rightWrist.y],
    );
    elbow.left = calculateMidAngle(
      [leftShoulder.x, leftShoulder.y],
      [leftElbow.x, leftElbow.y],
      [leftWrist.x, leftWrist.y],
    );
    armpit.right = calculateMidAngle(
      [rightElbow.x, rightElbow.y],
      [rightShoulder.x, rightShoulder.y],
      [rightHip.x, rightHip.y],
    );
    armpit.left = calculateMidAngle(
      [leftElbow.x, leftElbow.y],
      [leftShoulder.x, leftShoulder.y],
      [leftHip.x, leftHip.y],
    );
    ...
    if (
      status.current !== "up" &&
      poseAngles.armpit.right > 110 &&
      poseAngles.armpit.left > 110
    ) {
      status.current = "up";
    }
    if (
      status.current &&
      status.current === "up" &&
      poseAngles.armpit.right < 70 &&
      poseAngles.armpit.left < 70
    ) {
      status.current = "down";
